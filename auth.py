...
class AuthenticationDelegate(DefaultDelegate):
    ...
    def handleNotification(self, hnd, data):
        if hnd == self.device._char_auth.getHandle():
            ...
            self.device._log.error('Unhandled Response ' +
                                   hex(hnd) +
                                   ': ' +
                                   str(data.encode('hex')) +
                                   ' len:' +
...
class MiBand3(Peripheral):
    ...
    def _parse_raw_accel(self, data):
        res = []
        for i in xrange(3):
            g = struct.unpack('hhh', data[2 + i * 6:8 + i * 6])
            res.append({'x': g[0], 'y': g[1], 'wtf': g[2]})
        return res

    def _parse_raw_heart(self, data):

        res = struct.unpack('HHHHHHH', data[2:])
        return res

    def _parse_date(self, data):
        year = struct.unpack('h', data[0:2])[0] if len(data) >= 2 else None
        month = struct.unpack('b', data[2:3])[0] if len(data) >= 3 else None
        day = struct.unpack('b', data[3:4])[0] if len(data) >= 4 else None
        hours = struct.unpack('b', data[4:5])[0] if len(data) >= 5 else None
        minutes = struct.unpack('b', data[5:6])[
            0] if len(data) >= 6 else None
        seconds = struct.unpack('b', data[6:7])[
            0] if len(data) >= 7 else None
        day_of_week = struct.unpack(
            'b', data[7:8])[0] if len(data) >= 8 else None
        fractions256 = struct.unpack(
            'b', data[8:9])[0] if len(data) >= 9 else None
    ...
    def _parse_battery_response(self, data):
        level = struct.unpack('b', data[1:2])[0] if len(data) >= 2 else None
        last_level = struct.unpack('b', data[19:20])[
            0] if len(data) >= 20 else None
        status = 'normal' if struct.unpack(
            'b', data[2:3])[0] == 0 else 'charging'
        datetime_last_charge = self._parse_date(data[11:18])
        datetime_last_off = self._parse_date(data[3:10])
    ...
    def set_heart_monitor_sleep_support(
            self, enabled=True, measure_minute_interval=1):
    ...
    def get_steps(self):
        ...
        unused = struct.unpack('h', a[7:9])[0] if len(a) >= 9 else None
        fat_gramms = struct.unpack('h', a[3:5])[0] if len(a) >= 5 else None
        callories = struct.unpack('h', a[9:11])[0] if len(a) >= 11 else None
        return {
            'steps': steps,
            'meters': meters,
    ...
    def send_alert(self, notification_type):
        svc = self.getServiceByUUID(UUIDS.SERVICE_ALERT)
        char = svc.getCharacteristics(UUIDS.CHARACTERISTIC_ALERT)[0]
        char.write(notification_type)

    def send_custom_alert(self, notification_type):
        ...
        phone = input('Sender Name or Caller ID').encode('utf-8')
    ...
    def change_date(self):
        ...
        char.write(
            '\xe2\x07\x01\x1e\x00\x00\x00\x00\x00\x00\x16',
            withResponse=True)
    ...
    def start_raw_data_realtime(
            self,
            heart_measure_callback=None,
            heart_raw_callback=None,
            accel_raw_callback=None):
        char_m = self.svc_heart.getCharacteristics(
            UUIDS.CHARACTERISTIC_HEART_RATE_MEASURE)[0]
        char_d = char_m.getDescriptors(
            forUUID=UUIDS.NOTIFICATION_DESCRIPTOR)[0]
